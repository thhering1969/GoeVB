...
    -- Joine mit der 'inserted'-Tabelle, um NUR die aktuell betroffenen Zeilen zu aktualisieren.
    JOIN
        inserted AS i ON T.TargetID = i.TargetID
    WHERE
        -- Führt die Aktualisierung nur durch, wenn die Beschreibung sich ändert
        (T.OSDescription IS NULL OR T.OSDescription <> M.FullOSDescription);

END
GO

PRINT 'Trigger [trg_UpdateOSDescriptionOnInsertOrUpdate] wurde erfolgreich erstellt.';
GO
